...
```text
build/libs/grenadier-1.0.0.jar
```

2026-08-02 最近一次验证结果：编译成功，17 项自动化测试全部通过。
...
当前结论是：**“掷弹兵”已经是独立运行模组和可独立构建的源码工程；依赖边界已收口为客户端 Veil，仍需完成遗留兼容内容与发布资料审计。**

在第 7 节问题完成决策并重新执行 `:grenadier:check`、客户端启动测试及专用服务端启动测试前，不应把当前 Jar 标记为正式公开发行版。
